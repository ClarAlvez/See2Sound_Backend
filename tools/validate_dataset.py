...
print("\nDistribuição de labels positivas por crop:")
print(df[label_cols].sum(axis=1).describe())

print("\nLinhas com mais de 8 labels ativas:")
many = df[df[label_cols].sum(axis=1) > 8]
print(len(many))

if len(many) > 0:
    print(many[["frame_path"] + label_cols].head())