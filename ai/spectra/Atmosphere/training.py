...
def train_atmosphere_model(dataset_path, output_dir, config=None):
...
def train_single_split(dataset_path, output_dir, config):
    device = "cuda" if torch.cuda.is_available() else "cpu"

    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)

    checkpoint_path = output_dir / "atmosphere_net_best.pt"
    ...
    train_dataset = Subset(train_data, train_subset.indices)
    validation_dataset = Subset(validation_data, validation_subset.indices)
...
def train_cross_validation(dataset_path, output_dir, config):
    device = "cuda" if torch.cuda.is_available() else "cpu"
    ...
    print("=" * 80)
    print("TREINANDO SPECTRA ATMOSPHERE NET - CROSS VALIDATION")
    print("=" * 80)
    ...
    for fold_index in range(config.folds):
        ...
        validation_dataset = Subset(validation_data, validation_indices)
        ...
        fold_checkpoint_path = fold_output_dir / "atmosphere_net_best.pt"
...
def build_criterion(dataset, label_count, device, config):
...
def compute_pos_weight(dataset, label_count, device, max_pos_weight=8.0):
    positives = torch.zeros(label_count)
...
def update_history(history, train_loss, validation_loss, validation_metrics):
...
def save_training_plots(history, loss_plot_path, metrics_plot_path):
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.title("SpectraAtmosphereNet - Training Loss")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Score")
    plt.title("SpectraAtmosphereNet - Validation Metrics")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
...
def print_training_header(
    ...
    print("=" * 80)
    print("TREINANDO SPECTRA ATMOSPHERE NET")
    print("=" * 80)
    print("Dataset:", dataset_path)
    print("Output:", output_dir)
    print("Device:", device)
    ...
    print("Threshold:", config.threshold)
    print("Use pos weight:", config.use_pos_weight)
    print("=" * 80)
...
def main():
    parser = argparse.ArgumentParser(
        description="Treina a SpectraAtmosphereNet."
    )
    ...
    parser.add_argument(
        "--no-pos-weight",
        action="store_true",
        help="Desativa pos_weight no BCEWithLogitsLoss.",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=8.0,
        help="Valor máximo do pos_weight por label.",
    )
