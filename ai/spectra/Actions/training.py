...
def create_dataloaders(
    ...
    if dataset_size <= 2:
        raise ValueError(
            f"Dataset muito pequeno para treino/validação/teste: {dataset_size}"
        )
    ...
    validation_loader = DataLoader(
        Subset(validation_base_dataset, validation_indices),
        batch_size=config.batch_size,
        shuffle=False,
        num_workers=config.num_workers,
    )
    ...
    test_loader = DataLoader(
        Subset(test_base_dataset, test_indices),
        batch_size=config.batch_size,
        shuffle=False,
        num_workers=config.num_workers,
    )
...
def create_action_model(config: ActionTrainingConfig) -> SpectraActionNet:
    return SpectraActionNet(
...
def save_test_result(
    ...
    output_path.parent.mkdir(parents=True, exist_ok=True)
    pd.DataFrame([test_result]).to_csv(output_path, index=False)
...
def plot_training_history(
    ...
    pairs = [
        ("train_loss", "validation_loss", "loss.png", "Loss"),
        ("train_f1_score", "validation_f1_score", "f1_score.png", "F1-score"),
        ("train_precision", "validation_precision", "precision.png", "Precision"),
        ("train_recall", "validation_recall", "recall.png", "Recall"),
        ("train_hamming_accuracy", "validation_hamming_accuracy", "hamming_accuracy.png", "Hamming Accuracy"),
        ("train_exact_match_accuracy", "validation_exact_match_accuracy", "exact_match_accuracy.png", "Exact Match Accuracy"),
    ]

    for train_key, validation_key, filename, ylabel in pairs:
...
def train_action_model(
    dataset_path: str,
    output_dir: str,
    config: Optional[ActionTrainingConfig] = None,
) -> Dict[str, Any]:
    config = config or ActionTrainingConfig()
    ...
    model = create_action_model(config).to(device)

    criterion = nn.BCEWithLogitsLoss()
    optimizer = create_optimizer(model, config)

    history = ActionTrainingHistory()

    best_validation_f1 = -1.0
    best_model_path = output_dir / "action_net_best.pt"

    print("\nIniciando treino da SpectraActionNet")
    ...
    for epoch in range(config.epochs):
        ...
        history.add_epoch(train_result, validation_result)

        print(
            "Epoch {}/{} | Train Loss: {:.4f} | Val Loss: {:.4f} | Val F1: {:.4f} | Val Precision: {:.4f} | Val Recall: {:.4f}".format(
                epoch + 1,
                config.epochs,
                train_result["loss"],
    ...
    save_test_result(test_result, output_dir / "test_result.csv")
    plot_training_history(history.to_dict(), output_dir / "plots")
...
def build_training_config_from_args(args) -> ActionTrainingConfig:
    return ActionTrainingConfig(
...
def main():
    parser = argparse.ArgumentParser(
        description="Treina a SpectraActionNet."
    )
