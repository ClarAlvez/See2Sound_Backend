...
class ObjectPredictor:
    def __init__(self, model_path, threshold=0.5, top_k=None, device=None):
        ...
        if not self.model_path.exists(): raise FileNotFoundError(f"Modelo de objetos não encontrado: {self.model_path}")
        checkpoint = torch.load(self.model_path, map_location=self.device)
        self.labels = checkpoint.get("labels", LABELS)
        config = checkpoint.get("config", {})
        self.transform = get_test_transforms(config.get("image_size", 224))
        self.model = SpectraObjectNet(len(self.labels), config.get("image_size", 224), config.get("dropout_rate", 0.3), config.get("backbone_name", "resnet18"), pretrained=False, freeze_backbone=config.get("freeze_backbone", False)).to(self.device)
    ...
    def predict_frame(self, image_path, threshold=None, top_k=None, group_by_category=False):
        ...
        result = {"frame_path": str(image_path), "task_name": "object", "threshold": cutoff, "predictions": predictions}
        if group_by_category: result["grouped_predictions"] = {"scene": [], "person": [], "object": predictions}
        return result

    def predict_top_labels(self, image_path, top_k=10):
        result = self.predict_frame(image_path, threshold=0.0, top_k=top_k)
        return {"frame_path": result["frame_path"], "task_name": "object", "top_predictions": result["predictions"]}
